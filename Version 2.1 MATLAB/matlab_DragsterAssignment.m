function [telemetrylog, finishtime] = matlab_DragsterAssignment(online, plotting, python)
    ...
    %   2.1 Update: Can now run student_AutomaticGearbox.py by setting
    %   "python" input parameter to 1. 

    
    disp('Main Program Version 2.1')
        ...
        if(initialRun) % should ask for gear before a timestep is taken
            if(python)
                if car.dyn.gear == -1
                    ...
                    gear_arg = car.dyn.gear;
                end
                pyrunstring = "run_studentGearbox.py " + num2str(gear_arg) + " " + num2str(car.dyn.rpm) + " " +...
                num2str(car.dyn.acc) + " " + num2str(car.dyn.speed) + " " + num2str(car.dyn.throttle) + " " +...
                num2str(car.dyn.dist) + " " + num2str(car.dyn.time);
			    [gear_demand, throttle] = pyrunfile(pyrunstring, ["gear_demand", "throttle"]);
                gear_demand = double(gear_demand);
                throttle = double(throttle);
			    initialRun = 0;
            else
            ...
            if(python)
                if telemetry.Gear == -1
                    ...
                    gear_arg = telemetry.Gear;
                end
                pyrunstring = "run_studentGearbox.py " + num2str(gear_arg) + " " + num2str(telemetry.RPM) + " " +...
                num2str(telemetry.LongAcc) + " " + num2str(telemetry.Velocity) + " " + num2str(telemetry.Throttle) + " " +...
                num2str(telemetry.Distance) + " " + num2str(telemetry.TimeLap);
                [gear_demand, throttle] = pyrunfile(pyrunstring, ["gear_demand", "throttle"]); 
                gear_demand = double(gear_demand);
                throttle = double(throttle);
            else
